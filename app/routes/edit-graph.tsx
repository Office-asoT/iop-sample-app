...
import type { Route } from "./+types/edit-graph";

const buildPostData = (currentSetting, visible) => {
  const currentGraphSetting = currentSetting["graph"];
  const result = {
    "setting": currentSetting,
  };
  return Object.keys(currentGraphSetting).reduce((result, dataName) => {
    result["setting"]["graph"][dataName] = visible.includes(dataName);
    return result;
  }, result);
}
